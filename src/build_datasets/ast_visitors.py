    ...
    def visit_Call(self, node: ast.AST):
        ...
        try:
            if self._is_getattr_call(node):
                ...
                func, ast.BinOp
            ):  # for example "(ctypes.c_int64 * self._output_ndims[i])()"
                return
            elif isinstance(func, ast.BoolOp):  # for example "(_load or (lambda v: v))(value_)"
                return
            elif (
                func.id == "getattr"
            ):  # don't need to handle getattr() because it is handled in _is_getattr_call()
                return
    ...
    def visit_ImportFrom(self, node: ast.AST):
        ...
        if node.level:  # relative import, rebuild module
            ...
            if self.fpath_tuple[-1] == "__init__.py":
                file_module += (
                    ".__init__"  # fix the module level of __init__ when doing relative import
                )
...
class APIDefineVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        func_body_start_end_positions = (
            self._get_positon(node.body[body_index]) if len(node.body) > body_index else None
        )
